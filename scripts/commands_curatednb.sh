...
chmod -R 777 output
gpu=0
bash scripts/sweep_curatednb_stage1.sh ${gpu} depthreg local
exp=ours
bash scripts/sweep_curatednb_stage2.sh ${gpu} enhancer default full visibility_mask ${exp} depthreg 30000 local ${exp}
# exp=ablate_autonerf
# bash scripts/sweep_curatednb_stage2.sh ${gpu} enhancer default full_nod visibility_mask ${exp} default 30000 local ${exp}
# exp=ablate_fisherrf
# bash scripts/sweep_curatednb_stage2.sh ${gpu} enhancer default full_nod visibility_mask ${exp} default 30000 local ${exp}
chmod -R 777 .