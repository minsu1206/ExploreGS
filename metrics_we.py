...
from utils.loss_utils import ssim, get_dreamsim_score
...
def evaluate(model_paths, traj):

    full_dict = {}
    per_view_dict = {}
    full_dict_polytopeonly = {}
    per_view_dict_polytopeonly = {}
    print("[DEBUG] : metrics.py : ", model_paths)
    print("")
    ...
    for scene_dir in model_paths:
        ...
        with open(scene_dir + f"/results_{traj}.json", 'w') as fp:
            json.dump(full_dict[scene_dir], fp, indent=True)
        with open(scene_dir + f"/per_view_{traj}.json", 'w') as fp:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--render_traj', type=str, default="")
    ...
    traj = args.render_traj
    if args.stage1:
        evaluate(args.model_paths, traj)
    else:
        from utils.general_utils import multi_yaml_parsing_render
        ...
        args.model_paths = [os.path.join("output", save_expname, scene_name)]
        evaluate(args.model_paths, traj)
